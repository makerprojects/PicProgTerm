...
bool SerialPort::open(const std::string &deviceName, int speed)
{
    ...
    // workaround for COM > 9
    // see also: https://github.com/stephane/libmodbus/issues/197
    dev = std::string("\\\\.\\") + dev;
    handle = ::CreateFileA(dev.c_str(), GENERIC_READ | GENERIC_WRITE,0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
